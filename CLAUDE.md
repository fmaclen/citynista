...
The intersection model: at nodes with 3+ roads (and 2-segment corners bending sharper than 135°), roads do **not** continue through. Each segment is trimmed back to a stop line (`computeIntersectionTrims`): far enough that its mouth clears every crossing road's edge plus a fixed gap (reserved for future crosswalks) — for shallow-angle arms the pullback grows with 1/sin of the crossing angle so the mouth exits the other road's corridor, but only for arms no wider than the road they're clearing (a major road never yields to a narrow ramp/path). Every arm carries a per-side lane profile (`getArmProfile`: roadway edge plus outermost sidewalk/grass band, independently per side — cross-sections can be asymmetric or roadless). An asphalt patch connects only the road-bearing mouths with corner curves; one solid pavement plate polygon (every mouth joined by outer corner curves) seals the junction interior, so paths (sidewalk-only segments) join junctions as pavement, never asphalt. Grass never enters a junction — verges stop square at their stop lines. **Exception:** a 2-segment sharp corner where both sides have the same `lanesKey` keeps its full cross-section — every lane interval turns the corner as its own band between matched edge curves (`addCornerBands`), so medians and verges continue around bends. Gentle 2-segment bends of the same cross-section get continuous swept round joins (`addNodeJoins`). Two _different_ cross-sections continuing into each other form a **transition node**: both segments pull back from the node (taper length scales with the width change) and the gap renders as a ruled taper (`addTransition`) — the pavement plate and per-side roadway (inner road edge to outer) morph between the profiles, while grass and medians cross the gap only where the matching side carries the same strip, otherwise stopping square at their own mouth. Medians end at stop lines of 3+-arm junctions. `buildNodeLayers` applies the same junction-disc curb rounding as the full build — without it, slivers between patch, corner bands, and mouths leak the ground color.
