...
3. **Lane types** (`src/lib/core/lane-types.ts`): the descriptor table. Every lane type (road, concrete, bike, parking, transit, sidewalk, grass, median) declares a **surface class** (`roadway` / `island` / `verge` / `walkway`), color, render order, and directionality — and every rendering/connection rule is keyed to the class, never the type name. Adding a lane type is one row in this table: roadways flow into each other across transitions (material changes are a color seam), islands nose when nothing continues them, verges cut square, walkways render via the pavement plate.
